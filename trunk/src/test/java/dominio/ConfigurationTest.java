...
public class ConfigurationTest extends TestCase {
    ...
    public PartidoDeCopa crearPartidoDeCopa(final Equipo eqa, final Equipo eqb) {
        PartidoDeCopa p = new PartidoDeCopa(eqa, eqb);
        int cgA = new Random().nextInt(6);
        int cgB = new Random().nextInt(6);
        p.setGolesEquipoA(cgA);
        p.setGolesEquipoB(cgB);
        int cpA = new Random().nextInt(6);
        int cpB = new Random().nextInt(6);
        p.setPenalesConvertidosEquipoA(cpA);
        p.setPenalesConvertidosEquipoB(cpB);
        return p;
    ...
    public void testApp() throws Exception {
        ...
        try { // Ejemplo: ingresar 1001 jugadores en la tabla
              // jugador // for (int i = 0; i < 1000; i++) { // Jugador j = new Jugador("UnNombre" + i, "UnApellido",
              // i);
              // //
